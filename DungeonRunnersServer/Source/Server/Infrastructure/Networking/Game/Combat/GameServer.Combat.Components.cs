    ...
    {        private void HandleComponentUpdate(RRConnection conn, LEReader reader)
        {
            try
            {
                ...
                {
                    ZoneNPC merchantNpc = _zoneNPCs.TryGetValue(conn.CurrentZoneId, out List<ZoneNPC> buyZoneNpcs)
                ...
                {
                    bool isMerchant = FindMerchantNpcByComponentId(componentId) != null;
                ...
                {
                    ZoneNPC trainerNpc = FindTrainerNpcByComponentId(componentId);
        ...
        {
            if (conn == null || !_zoneNPCs.TryGetValue(conn.CurrentZoneId, out List<ZoneNPC> zoneNpcs))
                return null;
            return zoneNpcs.FirstOrDefault(npc => npc.Id == entityId);
        }

        private ZoneNPC FindMerchantNpcByComponentId(ushort componentId)
        {
            foreach (Zone zone in _zoneOrder)
            {
                if (!_zoneNPCs.TryGetValue(zone.id, out List<ZoneNPC> zoneNpcs))
                    continue;
                foreach (ZoneNPC npc in zoneNpcs)
        ...
        private ZoneNPC FindTrainerNpcByComponentId(ushort componentId)
        {
            foreach (Zone zone in _zoneOrder)
            {
                if (!_zoneNPCs.TryGetValue(zone.id, out List<ZoneNPC> zoneNpcs))
        ...
        private bool IsKnownTargetActionEntity(RRConnection conn, ushort targetEntityID)
        {
            ...
            if (_zoneNPCs.TryGetValue(conn.CurrentZoneId, out var npcs) && npcs.Any(n => n.Id == targetEntityID)) return true;
        ...
        private bool QueuePlayerUseTargetActionResponse(RRConnection conn, PendingPlayerUseTargetActionInput input, Combat.Monster monster, RRConnection targetPlayer)
        {
            if (conn == null
                ...
                return false;
            bool queuesPendingBehaviorAction = targetPlayer != null
        ...
        private void PromotePendingPlayerUseTargetBehaviorAction(uint playerEntityId, uint simulationTick)
        {
            ...
            if (!monsterAvailable && !playerAvailable)
            {
        ...
        private void ApplyPendingPlayerUsePositionActionInputs(uint simulationTick)
        {
            ...
            for (int pendingIndex = 0; pendingIndex < pendingSpellCount; pendingIndex++)
            {
                if (!_pendingSpells.TryDequeue(out PendingSpell pending))
                    break;
                if (!pending.ActionAdmissionApplied
                    && !pending.AwaitingActionResponsePacket
