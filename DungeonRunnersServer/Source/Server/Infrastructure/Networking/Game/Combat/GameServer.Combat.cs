    ...
    {private const int ClientUseTargetMirrorAdmissionTicks = 0;
        ...
        void Update()
        {
                ...
                && _pendingSimulationMilliseconds >= SIMULATION_TICK_INTERVAL_MILLISECONDS)
            {
                ...
                ApplyPendingPlayerUseTargetActionInputs(tickIndex);
                ApplyPendingPlayerUsePositionActionInputs(tickIndex);
                ApplyPendingSelfCastActionInputs(tickIndex);
                ApplyPendingOwnerClientControlInputs(tickIndex);
                ApplyPendingPlayerCancelActionInputs(tickIndex);
