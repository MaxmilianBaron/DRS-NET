...
public partial class GameServer
    {        private struct ItemDropPlacement
        ...
        private static bool UseTargetMovingWaypointReached(RRConnection conn, int waypointFixedX, int waypointFixedY, int speedPerFrameFixed, int steeringArrivalDistanceFixed)
        {
            long distanceSqFixed = DistanceSqFixed(conn.UseTargetMovingFixedX, conn.UseTargetMovingFixedY, waypointFixedX, waypointFixedY);
            long speedSqFixed = (long)speedPerFrameFixed * speedPerFrameFixed;
            long steeringSqFixed = (long)steeringArrivalDistanceFixed * steeringArrivalDistanceFixed;
            return distanceSqFixed <= speedSqFixed && distanceSqFixed <= steeringSqFixed;
        }
        ...
        private static void AdvanceExactUseTargetMovingWaypoints(RRConnection conn)
        {
            ...
            while (conn.UseTargetMovingPathIndex < conn.UseTargetMovingPathFixed.Count)
            {
                ...
                if (waypoint.X != conn.UseTargetMovingFixedX || waypoint.Y != conn.UseTargetMovingFixedY)
                    break;
        ...
        private bool UpdateUseTargetMovingSteering(RRConnection conn, PathMap pathMap, int speedPerFrameFixed, int steeringArrivalDistanceFixed)
        {
            ...
            {
                var waypoint = conn.UseTargetMovingPathFixed[conn.UseTargetMovingPathIndex];
                if (UseTargetMovingWaypointReached(conn, waypoint.X, waypoint.Y, speedPerFrameFixed, steeringArrivalDistanceFixed))
                {
                    ApplyUseTargetMovingPosition(
                        conn,
                        waypoint.X,
                        waypoint.Y,
                        conn.UseTargetMovingHeadingFixed,
                ...
                else
                {
                    ...
                    if (!pathMap.CastGroundRayBlockedFixed(
                        conn.UseTargetMovingFixedX,
                        conn.UseTargetMovingFixedY,
                        nextWaypoint.X,
                        nextWaypoint.Y))
                    {
                        conn.UseTargetMovingPathIndex++;
                        Debug.LogError($"[USE-TARGET-MOVING-PATH] conn={conn.ConnId} state=skip-waypoint waypoint=({waypoint.X},{waypoint.Y}) next=({nextWaypoint.X},{nextWaypoint.Y}) sourceFunction=UnitMover::UpdateSteering@0x00536380");
                    }
                }
            }
            int waypointFixedX = conn.UseTargetMovingPathTargetFixedX;
            int waypointFixedY = conn.UseTargetMovingPathTargetFixedY;
            if (conn.UseTargetMovingPathIndex < conn.UseTargetMovingPathFixed.Count)
            {
                waypointFixedX = conn.UseTargetMovingPathFixed[conn.UseTargetMovingPathIndex].X;
                waypointFixedY = conn.UseTargetMovingPathFixed[conn.UseTargetMovingPathIndex].Y;
            }
            ...
            if (conn.UseTargetMovingPathRetryCountdown == 0)
            {
                conn.UseTargetMovingPathRetryCountdown = 15;
                return true;
            }
            conn.UseTargetMovingPathRetryCountdown--;
            if (conn.UseTargetMovingPathRetryCountdown != 0)
                return true;
        ...
        private void ApplyUseTargetMovingPosition(RRConnection conn, int fixedX, int fixedY, int headingFixed, PathMap pathMap, int? resolvedGroundFixedZ = null)
        {
            ...
            bool ownerActionMoverOwnsUnitMover = ActionMirroredMoveToPointOwnsUnitMover(conn);
            bool canonicalActionMoverOwnsUnitMover = CanonicalActionMirroredMoveToPointOwnsUnitMover(conn);
        ...
        private void TickActionMirroredUseTargetMoving(RRConnection conn, PlayerState state, CombatPlayer player, uint tickIndex, int targetFixedX, int targetFixedY)
        {
            ...
            if (conn.UseTargetMovingRetargetCountdown == 0)
            {
                var monster = CombatRuntime.Instance.GetMonster(conn.ActiveUseTargetId)
                           ?? CombatRuntime.Instance.GetMonsterByComponent(conn.ActiveUseTargetId);
                bool targetMovingThisFrame = CombatRuntime.Instance.IsMonsterClientVisibleMovingThisFrame(monster);
            ...
            int steeringPathIndex = conn.UseTargetMovingPathIndex;
            int moveTargetX = conn.UseTargetMovingPathTargetFixedX;
            int moveTargetY = conn.UseTargetMovingPathTargetFixedY;
            if (steeringPathIndex < conn.UseTargetMovingPathFixed.Count)
            {
                moveTargetX = conn.UseTargetMovingPathFixed[steeringPathIndex].X;
                moveTargetY = conn.UseTargetMovingPathFixed[steeringPathIndex].Y;
            }
            if (!UpdateUseTargetMovingSteering(conn, pathMap, avatarStepFixed, steeringArrivalDistanceFixed))
            {
            ...
            if (conn.UseTargetMovingPathRequestId > 0)
                return;
            bool steeringAdvancedPath = conn.UseTargetMovingPathIndex != steeringPathIndex;
            ...
            if (!arrived)
                return;
            if (steeringAdvancedPath)
                return;
            if (conn.UseTargetMovingPathFixed.Count > 0 && conn.UseTargetMovingPathIndex < conn.UseTargetMovingPathFixed.Count - 1)
            {
                conn.UseTargetMovingPathIndex++;
                return;
            }
            ...
            conn.OwnerUnitBehaviorMoverMode = UnitMover.StoppedMode;
            SyncReflectedAvatarCombatPosition(conn);
            Debug.LogError($"[USE-TARGET-MOVING] state=2-mover-stop conn={conn.ConnId} target={conn.UseTargetMovingEntityId} sourceFunction=UnitMover::StopMoving@0x00536190");
        }
        ...
        private void TickActionMirroredActivateMoving(RRConnection conn, PlayerState state, CombatPlayer player, uint tickIndex, int targetFixedX, int targetFixedY)
        {
            ...
            int prevZ = conn.UseTargetMovingFixedZ;
            int prevHeading = conn.UseTargetMovingHeadingFixed;
            if (conn.UseTargetMovingMoverInitialUpdatePending)
            {
                int targetHeading = UnitMover.VectorToHeadingFixed(moveTargetX - prevX, moveTargetY - prevY);
                int initialHeading = UnitMover.InterpolateHeading(prevHeading, targetHeading, USE_TARGET_AVATAR_TURN_RATE_FIXED);
                conn.UseTargetMovingMoverInitialUpdatePending = false;
                if (initialHeading != prevHeading)
                    ApplyUseTargetMovingPosition(conn, prevX, prevY, initialHeading, pathMap, prevZ);
                return;
            }
            UnitMover.StepTowardFixedHeading(
                prevX,
            ...
            conn.OwnerUnitBehaviorMoverMode = UnitMover.StoppedMode;
            SyncReflectedAvatarCombatPosition(conn);
            Debug.LogError($"[ACTIVATE-MOVING] state=2-mover-stop conn={conn.ConnId} target={conn.UseTargetMovingActivateTargetId} tick={tickIndex} sourceFunction=UnitMover::StopMoving@0x00536190");
        }
        ...
        private void ApplyPendingOwnerClientControlInputs(uint simulationTick)
        {
            ...
            {
                if (input.SimulationApplyTick > simulationTick)
                {
