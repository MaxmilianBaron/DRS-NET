...
namespace DungeonRunners.Networking
{
    public partial class GameServer
    {
        ...
        private bool TryAcquireDroppedGoldTransaction(RRConnection conn, ushort entityId, uint goldAmount, string source, out DroppedItemInfo info, out uint updatedGold)
        {
            info = null;
            updatedGold = 0;
            ...
            lock (_droppedItems)
            {
                if (!_droppedItems.TryGetValue(entityId, out info)
                    ...
                    || !_droppedGoldAcquisitions.Add(entityId))
                    return false;
            }
        ...
        public bool BlingGnomeAcquireItemAsGold(RRConnection conn, ushort entityId, uint goldAmount)
        {
            if (!TryAcquireDroppedGoldTransaction(conn, entityId, goldAmount, "bling-gnome", out DroppedItemInfo info, out uint updatedGold))
                return false;
        ...
        {
            if (monster == null || !_pendingMonsterDeathExperience.TryGetValue(monster.EntityId, out var pending))
                return;

            _pendingMonsterDeathExperience.Remove(monster.EntityId);
            if (pending.monster != monster)
                return;
        ...
        private void OnMonsterFollowReachedNearStop(Monster monster, CombatPlayer target)
        {
            if (monster == null || target == null || !monster.IsAlive || !target.IsAlive)
                return;
            RRConnection targetConn = FindConnectionByAvatarEntityId(target.EntityId);
        ...
        private void SupersedePendingMonsterFollowWithAttack(Monster monster, CombatPlayer target)
        {
            bool removed = false;
        ...
        private void ApplyPendingMonsterBehaviorInputs(uint simulationTick)
        {
            ...
            while (updateIndex < _pendingMonsterBehaviorUpdates.Count)
            {
                ...
                if (update.SimulationApplied)
                {
                    if (update.PacketSent && update.WireSimulationApplyTick != 0)
                    {
                            ...
                            && simulationTick >= update.WireSimulationApplyTick)
                        {
                            ...
                            CombatPlayer wireTarget = CombatRuntime.Instance.GetPlayer(update.TargetEntityId);
                ...
                {
                    CombatPlayer target = CombatRuntime.Instance.GetPlayer(update.TargetEntityId);
                    committed = update.UseTargetAction
                        && update.PacketSent
        ...
        private void FlushPendingMonsterAttackUpdate(PendingMonsterBehaviorUpdate update, uint writerTick)
        {
            ...
            if (!update.SimulationApplied && update.SimulationApplyTick == 0)
                update.SimulationApplyTick = update.UseTargetAction ? writerTick : unchecked(writerTick + 1);
            update.PacketSent = true;
        ...
        private void StartGameRuntime()
        {
            ...
            QuestManager.Instance.SetZoneQuestGiversCallback(conn =>
                _zoneNPCs.TryGetValue(conn.CurrentZoneId, out List<ZoneNPC> npcs)
        ...
        private void SendMonsterLiveSnapshotToClient(RRConnection conn, Monster monster)
        {
            ...
            byte[] packet = CombatPackets.BuildMonsterLiveSnapshotPacket(
                ...
                monster.ModifiersId,
                snapshot,
                decision.ToResolved(monster.EntityId, monster.BehaviorId, 0x04, "MON-LIVE-SNAPSHOT"));
            if (!QueueClientEntityStream(conn, packet))
                return;
