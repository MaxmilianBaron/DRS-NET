                ...
                foreach (var grant in grants)
                {
                    try
                    {
                        ...
                        if (onlineConn != null && onlineConn.UnitContainerId != 0)
                        {
                            ...
                            GeneralItemData authoredItem = AuthoredGameplayCatalog.FindGeneralItem(gcTypeFull);
                            ...
                            gcTypeFull = GCObject.GetPacketGCClassFor(authoredItem.gcType);
                        ...
                        {
                            string offlineGcType = grant.gcClass;
                            if (!offlineGcType.StartsWith("items.", StringComparison.OrdinalIgnoreCase))
                                offlineGcType = "items.pal." + offlineGcType;
                            GeneralItemData offlineAuthoredItem = AuthoredGameplayCatalog.FindGeneralItem(offlineGcType);
                            ...
                            offlineGcType = GCObject.GetPacketGCClassFor(offlineAuthoredItem.gcType);
        ...
        private void ApplyServerReconstructionScaling(Monster monster)
        {
            ...
            if (!IsPublicZone(monster.ZoneName))
            {
                ...
                if (group != null)
                {
                    ...
                    difficulty = Math.Clamp(group.MonsterDifficulty, (byte)0, (byte)3);
                }
            }

            ServerEncounterScale scale = ServerReconstructionPolicy.ResolveEncounterScale(partySize, difficulty);
            uint originalMaxHPWire = monster.MaxHPWire;
            uint originalCurrentHPWire = monster.CurrentHPWire;
            int originalDamagePercent = monster.DamageModPercent;
            int originalExperienceValueMultF32 = monster.ExperienceValueMultF32;
            monster.MaxHPWire = ServerReconstructionPolicy.ScaleWireValue(monster.MaxHPWire, scale.HealthPercent);
            monster.CurrentHPWire = originalMaxHPWire == 0
                ? monster.MaxHPWire
                : Math.Min(monster.MaxHPWire, (uint)Math.Max(1L, ((long)originalCurrentHPWire * monster.MaxHPWire + originalMaxHPWire / 2L) / originalMaxHPWire));
            monster.DamageModPercent = ServerReconstructionPolicy.ScaleNonNegative(monster.DamageModPercent, scale.DamagePercent);
            monster.ExperienceValueMultF32 = ServerReconstructionPolicy.ScaleNonNegative(monster.ExperienceValueMultF32, scale.ExperiencePercent);
            monster.ServerReconstructionPartySize = (byte)scale.PartySize;
            monster.ServerReconstructionDifficulty = (byte)scale.Difficulty;
            monster.ServerReconstructionGoldPercent = scale.GoldPercent;
            monster.ServerReconstructionItemLevelBonus = scale.ItemLevelBonus;
            monster.ServerReconstructionScalingApplied = true;
            CombatRuntime.Instance.SetMonsterHPWire(monster, monster.CurrentHPWire, false, "server-reconstruction-scale");
            Debug.LogError($"[SERVER-RECONSTRUCTION] area=spawn instance='{monsterInstanceKey}' monster={monster.Name}#{monster.EntityId} tier='{monster.Tier ?? ""}' partySize={scale.PartySize} difficulty={scale.Difficulty} hpPercent={scale.HealthPercent} hpWire={originalMaxHPWire}->{monster.MaxHPWire} damagePercent={scale.DamagePercent} damageMod={originalDamagePercent}->{monster.DamageModPercent} experiencePercent={scale.ExperiencePercent} experienceValueMultF32={originalExperienceValueMultF32}->{monster.ExperienceValueMultF32} goldPercent={scale.GoldPercent} itemLevelBonus={scale.ItemLevelBonus} policy=conservative-v1");
        }
