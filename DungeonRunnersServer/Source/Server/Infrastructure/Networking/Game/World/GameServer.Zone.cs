...
namespace DungeonRunners.Networking
{
    public partial class GameServer
    {
        ...
        private void ApplyAvatarRuntimeProperties(GCObject avatar, SavedCharacter savedChar, PlayerState playerState)
        {
            ...
            SetAvatarUInt32Property(avatar, "MaxTotalAttributePool", checked((uint)maxTotalAttributePool));
            SetAvatarUInt32Property(avatar, "PVPRating", checked((uint)Math.Max(0, savedChar.pvpRating)));
        }
        ...
        private static ZonePortal BuildProceduralZonePortal(DungeonMazeSpawner.ProceduralDungeonSnapshot snapshot, DungeonPortalRole role)
        {
            if (snapshot == null)
                return null;

            bool entry = role == DungeonPortalRole.Entry;
            string targetZone = entry ? snapshot.EntryLinkToZone : snapshot.ExitLinkToZone;
            string spawnPoint = entry ? snapshot.EntryLinkToSpawn : snapshot.ExitLinkToSpawn;
            if (string.IsNullOrWhiteSpace(targetZone) || string.IsNullOrWhiteSpace(spawnPoint))
                return null;

            string gcType = entry ? snapshot.EntryPortalGcType : snapshot.ExitPortalGcType;
            if (string.IsNullOrWhiteSpace(gcType))
                gcType = targetZone.Equals("town", StringComparison.OrdinalIgnoreCase)
                    || targetZone.Equals("tutorial", StringComparison.OrdinalIgnoreCase)
                    ? "misc.zoneportal_hub"
                    : "misc.zoneportal_agg";
            ...
            return new ZonePortal
            {
                Id = 0,
                GCType = gcType,
                Name = entry ? "ZonePortal" : "ZonePortal_agg",
        ...
        private static bool HasPortalRoute(IEnumerable<ZonePortal> portals, ZonePortal candidate)
        {
            if (candidate == null || portals == null)
                return false;
            return portals.Any(portal => portal != null
                && string.Equals(portal.TargetZone, candidate.TargetZone, StringComparison.OrdinalIgnoreCase)
                && string.Equals(portal.SpawnPoint, candidate.SpawnPoint, StringComparison.OrdinalIgnoreCase));
        }

        private List<ZonePortal> ResolveZonePortalsForConnection(RRConnection conn, List<ZonePortal> portals)
        {
            var resolved = new List<ZonePortal>(portals?.Count ?? 0);
            bool hasSnapshot = TryGetProceduralDungeonSnapshot(conn, out var snapshot);
            if (portals != null)
            {
                foreach (var portal in portals)
                {
                    ...
                    if (hasSnapshot)
                    {
                        ...
                        ResolveDungeonPortalAnchor(snapshot, role, out int positionFixedX, out int positionFixedY, out int positionFixedZ, out int headingFixed, out int sourceIndex, out string tileType, out int gridX, out int gridY, out int localFixedX, out int localFixedY, out int localFixedZ, out string source);
                        clone.PosFixedX = positionFixedX;
                        clone.PosFixedY = positionFixedY;
                        clone.PosFixedZ = positionFixedZ;
                        clone.HeadingFixed = headingFixed;
                        Debug.LogError($"[DUNGEON-PORTAL] zone={snapshot.ZoneName} layoutSeed=0x{snapshot.LayoutSeed:X8} roomSeed=0x{snapshot.RoomSeed:X8} name={clone.Name} gc={clone.GCType} target={clone.TargetZone} spawnPoint={clone.SpawnPoint} role={role.ToString().ToLowerInvariant()} src={sourceIndex} tile='{tileType}' grid=({gridX},{gridY}) localFixed=({localFixedX},{localFixedY},{localFixedZ}) posFixed=({clone.PosFixedX},{clone.PosFixedY},{clone.PosFixedZ}) snapApplied=False headingFixed={clone.HeadingFixed} source='{source}'");
                    }
                    resolved.Add(clone);
                }
            }
            ...
            if (hasSnapshot)
            {
                foreach (DungeonPortalRole role in new[] { DungeonPortalRole.Entry, DungeonPortalRole.Exit })
                {
                    ZonePortal fallback = BuildProceduralZonePortal(snapshot, role);
                    if (fallback == null || HasPortalRoute(resolved, fallback))
                        continue;
                    ResolveDungeonPortalAnchor(snapshot, role, out int positionFixedX, out int positionFixedY, out int positionFixedZ, out int headingFixed, out int sourceIndex, out string tileType, out int gridX, out int gridY, out int localFixedX, out int localFixedY, out int localFixedZ, out string source);
                    fallback.PosFixedX = positionFixedX;
                    fallback.PosFixedY = positionFixedY;
                    fallback.PosFixedZ = positionFixedZ;
                    fallback.HeadingFixed = headingFixed;
                    resolved.Add(fallback);
                    Debug.LogError($"[DUNGEON-PORTAL] zone={snapshot.ZoneName} layoutSeed=0x{snapshot.LayoutSeed:X8} roomSeed=0x{snapshot.RoomSeed:X8} name={fallback.Name} gc={fallback.GCType} target={fallback.TargetZone} spawnPoint={fallback.SpawnPoint} role={role.ToString().ToLowerInvariant()} src={sourceIndex} tile='{tileType}' grid=({gridX},{gridY}) localFixed=({localFixedX},{localFixedY},{localFixedZ}) posFixed=({fallback.PosFixedX},{fallback.PosFixedY},{fallback.PosFixedZ}) snapApplied=True headingFixed={fallback.HeadingFixed} source='{source}'");
                }
            }
            return resolved;
        }
