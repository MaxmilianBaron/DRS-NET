            ...
            for (int attempt = 0; attempt < CharacterReadRetryCount; attempt++)
            {
                try
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        if (character.level == 0 && character.experience == 0 && character.gold == 0)
                        {
                            character.gold = 100;
                            GameDatabase.ExecuteNonQuery(connection,
                                "UPDATE characters SET gold = @gold WHERE id = @id AND level = 0 AND experience = 0 AND gold = 0",
                                ("@gold", 100), ("@id", (int)character.id));
                            Debug.LogError($"[DB-CHAR] initialized starting gold characterId={character.id} gold={character.gold}");
                        }
