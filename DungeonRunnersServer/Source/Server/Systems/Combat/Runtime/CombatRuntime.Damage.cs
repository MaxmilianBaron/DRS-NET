...
public partial class CombatRuntime
    {        private bool HasEnemyWithinMonsterPerception(Monster monster)
        ...
        {
            if (monster == null || !WanderSimulator.Instance.TryGetSnapshot(monster.EntityId, out WanderStateSnapshot wander))
                return false;
            if (wander.FixedInit)
            {
        ...
        private bool AggroMonster(Monster monster, CombatPlayer player, string reason, bool alignForCombat, bool queueFollowBehaviorInput = true, uint followInputApplyTick = uint.MaxValue, uint directPlayerWeaponHitTick = 0, uint playerDamageTick = 0)
        {
        ...
        private void PropagateMonsterShout(Monster source, CombatPlayer player, bool notifyBehaviorThreat = true, uint proximityAssistApplyTick = uint.MaxValue)
        {
        ...
        private bool AdmitMonsterAssist(Monster monster, Monster alertSource, CombatPlayer target, string relation, bool queueFollow, string source, bool notifyBehaviorThreat = true, uint followInputApplyTick = uint.MaxValue, uint directPlayerWeaponHitTick = 0, uint playerDamageTick = 0)
        {
        ...
        private bool ClearMonsterTargetStateForLostPlayer(Monster monster, uint lostPlayerEntityId, string source)
        {
            ...
            bool targetMissing = runtimeTargetId == 0 || !_players.ContainsKey(runtimeTargetId);
        ...
        public Monster SpawnMonsterFixed(string gcType, int posFixedX, int posFixedY, int posFixedZ, int headingFixed = 0, string zoneName = null, string encounterGroupKey = null, int encounterDifficultyF32 = 0x100, string spawnGcTypeOverride = null, string instanceKey = null, int encounterLevelOffset = 0)
        {
            var creatureData = AuthoredGameplayCatalog.FindCreature(gcType);
            ...
            MeleeBehaviorContext spawnBehaviorContext = BuildMeleeBehaviorContext(
                spawnRuntime.RoomRng,
                monster,
                null,
                int.MaxValue,
                ResolveMonsterEffectiveAttackRangeFixed(monster));
            monster.Behavior.EnterSimulation(spawnBehaviorContext);
            monster.UnitFlags |= 0x404;
            monster.StockUnitState = 8;
            InitializeMonsterState0Idle(monster, spawnRuntime.RoomRng);
