...
public partial class CombatRuntime
    {        public bool IsMonsterClientVisibleMovingThisFrame(Monster monster)
        ...
        private void RaisePlayerModifierAddWithId(Monster sourceMonster, CombatPlayer target, string modifierKey, string gcType, uint id, byte level, uint powerLevel, uint durationTicks, byte sourceIsSelf, bool replace, string skillPath, string effectPath, string source, string sourceFunction)
        {
        ...
        private bool RaisePlayerModifierAdd(Monster sourceMonster, CombatPlayer target, string modifierKey, string gcType, byte level, uint powerLevel, uint durationTicks, byte sourceIsSelf, bool replace, string skillPath, string effectPath, string source, string sourceFunction, bool trackModifierId = true)
        {
            if (target == null || string.IsNullOrWhiteSpace(gcType))
                return false;
        ...
        private bool ApplyPlayerDamageModifierFromMonster(Monster sourceMonster, CombatPlayer target, MonsterDamageModifierSkillEffect effect, string source, bool auraRefresh = false)
        {
            ...
            long modifierOrder = replace ? existing.Order : ++_nextPlayerModifierOrder;
        ...
        public MonsterModifierApplyResult AdvancePlayerDamageModifierRuntime(uint onlyTargetEntityId, uint simulationTick, string source)
        {
            ...
            foreach (string key in keys)
            {
                ...
                if (expiredBeforeTick && (mod.MaxTicks == 0 || mod.TicksApplied >= mod.MaxTicks || mod.NextTick > mod.ExpireTick))
                {
                    ...
                    _activePlayerDamageModifiers.Remove(key);
                    Debug.LogError($"[PLAYER-EFFECTMOD] complete target={target.Name}#{target.EntityId} source={sourceMonster.Name}#{sourceMonster.EntityId} ticks={mod.TicksApplied}/{mod.MaxTicks} hp={target.PlayerState.CurrentHPWire} reason=duration-expired expireTick={mod.ExpireTick} nowTick={nowTick} source={source ?? "unknown"} sourceFunction=Modifier::update@0x004FF1B0 EffectMod::update@0x0055FE70");
                    continue;
                }
                ...
                while (dueTicksProcessed == 0 && nowTick >= mod.NextTick && mod.TicksApplied < mod.MaxTicks && mod.NextTick <= mod.ExpireTick && target.IsAlive && target.PlayerState.CurrentHPWire > 0)
                {
                    dueTicksProcessed++;
                    ...
                    if (string.Equals(mod.AttackType, "RANGED", StringComparison.OrdinalIgnoreCase))
                        DamageResolver.ComputeRangedDamageRange(mod.SourceLevel, Math.Max(1, mod.SourceAgility), 0x100, mod.DamageModF32, mod.DamageVolatilityF32, out minDamage, out maxDamage);
                    else
                        DamageResolver.ComputeSpellDamageRange(mod.SourceLevel, Math.Max(1, mod.SourceIntellect), mod.DamageModF32, mod.DamageVolatilityF32, out minDamage, out maxDamage);
                    ...
                    target.PlayerState.DoAttributeModifierDamageEvent(modRng, target.EntityId, target.Name, source ?? "EffectMod::applyEffect");
                    if (afterHP == 0)
                        TryCommitPlayerAttributeModifierDeathEvent(target, source ?? "EffectMod::target-death");

                    string resultName = applied ? "HIT" : query.ResultName;
                    Debug.LogError($"[PLAYER-EFFECTMOD-TICK] target={target.Name}#{target.EntityId} source={sourceMonster.Name}#{sourceMonster.EntityId} tick={tickIndex}/{mod.MaxTicks} result={resultName} skill={mod.SkillPath} modifier={mod.ModifierPath} effect={mod.ModifierEffectPath} attackType={mod.AttackType} damageType={mod.DamageType} damageTypeId={mod.DamageTypeId} damageKind={mod.DamageKind} chanceWire={chanceWire} chanceRaw=0x{chanceRaw:X8} chanceRoll={chanceRoll} preQueryWire={damageWire} damageWire={adjustedDamageWire} hp={beforeHP}->{afterHP}/{target.PlayerState.MaxHPWire} range=[{minDamage},{maxDamage}] damageRaw=0x{damageRaw:X8} effectRaw=0x{effectRaw:X8} resist={DamageResistLogCode(query)} dueTick={mod.NextTick} nowTick={nowTick} rngAfter={modRng.CallsSinceReseed} sourceFunction=EffectMod::applyEffect@0x0055FF70 SpellDamageEffect::doEffect@0x0054FD20");
                    ...
                    if (applied)
                    {
                        ...
                        resolvedAfterHP = target.PlayerState.CurrentHPWire;
                        target.IsAlive = resolvedAfterHP > 0;
                    }
                    if (applied)
                        OnPlayerDamageResolved?.Invoke(sourceMonster, target, true, resolvedAfterHP, source != null ? $"EffectMod::applyEffect:{source}" : "EffectMod::applyEffect");
        ...
        public MonsterModifierApplyResult ApplyProjectileModifierFromSpell(Monster target, uint sourceEntityId, PlayerState sourceState, SpellData spell, MersenneTwister rng, int skillLevel, string source)
        {
            ...
            long modifierOrder = replace ? existingModifier.Order : ++_nextMonsterModifierOrder;
        ...
        private MonsterModifierApplyResult AdvanceMonsterModifierRuntime(MersenneTwister rng, uint simulationTick, string source, uint onlyTargetEntityId = 0)
        {
            var aggregate = new MonsterModifierApplyResult
            {
        ...
        public void NotifyDeferredMonsterOnAttackedThreat(Monster monster, uint playerEntityId, string reason)
        {
            if (monster == null || playerEntityId == 0 || !monster.IsAlive)
                return;
            if (!_players.TryGetValue(playerEntityId, out CombatPlayer player) || player == null || !player.IsAlive)
                return;
