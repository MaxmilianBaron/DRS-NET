...
public partial class CombatRuntime
    {        private void BeginMonsterFollowFindLineOfSightMove(Monster monster)
        ...
        {
            if (monster == null || !_players.TryGetValue(monster.TargetId, out CombatPlayer target) || target == null || !target.IsAlive)
                return 0;
        ...
        private int ScoreMonsterRetreatLocationFixed(Monster monster, int headingFixed, int distanceFixed, out int fixedX, out int fixedY)
        {
            ...
            long scoreFixed = ((long)distanceScoreFixed * proximityWeightFixed) >> 8;
            int score = scoreFixed > int.MaxValue ? int.MaxValue : (int)Math.Max(0, scoreFixed);
            if (FindAnyCollidingUnitFixed(monster, target, fixedX, fixedY))
        ...
        private void UpdateMonsterAttackSearchMoveForTarget(Monster monster)
        {
            if (monster == null || !monster.SearchForAttackMoveActive
                || !_players.TryGetValue(monster.TargetId, out CombatPlayer target)
        ...
        private bool FindAnyCollidingUnitFixed(Monster scanUnit, CombatPlayer viewer, int candidateFixedX, int candidateFixedY)
        {
            ...
            foreach (uint entityId in _entityOrder)
            {
                ...
                if (_players.TryGetValue(entityId, out CombatPlayer player))
                {
                    ...
                    if (UnitMover.IsMoving(moverMode))
                        continue;
        ...
        private bool DispatchMonsterBehaviorUpdateTargets(MeleeBehaviorContext context)
        {
            ...
            CombatPlayer target = FindMonsterBehaviorPrimaryTarget(monster);
            bool acquired = false;
            if (target != null && previousTargetEntityId == 0)
                ...
                _players.TryGetValue(previousTargetEntityId, out target);
        ...
        private CombatPlayer FindMonsterBehaviorPrimaryTarget(Monster monster)
        {
            ...
            var candidates = new List<(CombatPlayer Player, long DistanceSquaredFixed8, int Aggro)>();
            foreach (CombatPlayer player in GetPlayersInEntityOrder())
            {
                ...
                if (!MatchesInstance(monster, player.InstanceKey))
                    continue;
                if (player.PlayerState.CurrentHPWire == 0 && player.PlayerState.EntitySynchInfoHP == 0)
                    continue;
            ...
            foreach ((CombatPlayer Player, long DistanceSquaredFixed8, int Aggro) candidate in candidates)
            {
        ...
        private static int ResolveUnitFinderPlayerAggro(Monster monster, CombatPlayer player, long distanceSquaredFixed8)
        {
            ...
            if (distanceSquaredFixed8 < 160000)
                ...
                score -= 0x4B00;
            score += (long)state.AggroIncreaseBonus * 0x100;
            int aggroIncreaseModPercent = state.AggroIncreaseModPercent + state.GetActiveAttributeModifierValue("AGGRO_MOD");
            ...
            out uint targetEntityId)
        {
            ...
            if (sourcePrimaryTargetEntityId != 0)
                _players.TryGetValue(sourcePrimaryTargetEntityId, out target);
            if (source == null
        ...
        private bool DispatchMonsterBehaviorThreatNear(Monster monster)
        {
            if (monster == null || monster.TargetId == 0)
                return false;
            if (!_players.TryGetValue(monster.TargetId, out CombatPlayer target) || target == null || !target.IsAlive)
                return false;
        ...
        private MeleeBehaviorContext BuildMeleeBehaviorContext(MersenneTwister rng, Monster monster, CombatPlayer target, int distFixed, int allowedRangeFixed)
        {
            ...
            context.TargetPresent = target != null && target.IsAlive && target.PlayerState != null;
        ...
        private MeleeBehaviorContext BuildMeleeBehaviorContextForMonster(Monster monster)
        {
            ...
            uint targetEntityId = ResolveMonsterActionTargetEntityId(monster);
            if (targetEntityId != 0
                && _players.TryGetValue(targetEntityId, out CombatPlayer resolvedTarget)
                && resolvedTarget != null
        ...
        private void NotifyMonsterBehaviorThreatInput(Monster monster, CombatPlayer target, bool alternate)
        {
        ...
        private bool BeginMonsterFollowBehavior(Monster monster, CombatPlayer target)
        {
        ...
        private void QueueMonsterFollowBehaviorInput(Monster monster, CombatPlayer target, uint applyTick = uint.MaxValue, uint directPlayerWeaponHitTick = 0, uint playerDamageTick = 0)
        {
        ...
        public bool CommitMonsterFollowBehaviorInput(Monster monster, CombatPlayer target, uint applyTick)
        {
        ...
        private Behavior.MonsterBehavior2.ActionSlot ResolveMonsterAttackAction(Monster monster)
        {
            ...
            if (!_players.TryGetValue(monster.TargetId, out CombatPlayer target)
        ...
        private bool TryValidateMonsterActiveSkillTarget(Monster monster, MonsterActiveSkillRuntime skill, CombatPlayer target)
        {
            if (monster == null || skill == null || target == null || !monster.IsAlive || !target.IsAlive || target.PlayerState == null)
                return false;
        ...
        public bool DoMonsterUseTargetAction(Monster monster, bool weaponInterruptInput = false)
        {
            ...
            if (!_players.TryGetValue(targetEntityId, out CombatPlayer target) || target == null || !target.IsAlive)
                return false;
            if (monster.AttackPending)
            {
            ...
            int actualRangeF32 = ResolveMonsterSkillTargetClearRangeF32(monster, monster.SelectedActiveSkill);
        ...
        private bool ValidateMonsterWeaponUseTargetAction(Monster monster, CombatPlayer target, string sourceFunction)
        {
            if (monster == null
                || target == null
                || !monster.IsAlive
        ...
        public bool DoMonsterAttackAction(Monster monster)
        {
            ...
            if (monster.AttackPending)
                return true;
            if (!_players.TryGetValue(monster.TargetId, out CombatPlayer target) || target == null || !target.IsAlive || target.PlayerState == null)
            {
        ...
        private bool BeginMonsterRuntimeAttackAction(Monster monster, CombatPlayer target, bool useActiveSkill, string source)
        {
