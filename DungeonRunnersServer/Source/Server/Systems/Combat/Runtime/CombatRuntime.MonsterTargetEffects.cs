    ...
    public partial class CombatRuntime
    {
            ...
            out string reason)
        {
            ...
            foreach (GCNode attribute in attributes)
            {
                ...
                authoredAttributes[attributeName] = ResolveMonsterAttributeLevelValue(attribute, skillLevel);
            }
            ...
            out bool deferred)
        {
            ...
            hpShifted = false;
            deferred = false;
            if (monster == null || target?.PlayerState == null)
                return false;
            ...
            out bool deferred)
        {
            ...
            if (plan.Kind == MonsterTargetEffectNodeKind.EnemyAoe)
            {
                if (!PassesMonsterTargetEffectChance(sourceMonster, target, plan.EffectPath, plan.ChanceWire))
                    return true;
                List<CombatPlayer> targets = CollectMonsterEnemyAoeTargets(sourceMonster, target, plan.RadiusF32, plan.MaxTargets);
                bool handled = true;
                foreach (CombatPlayer aoeTarget in targets)
                    foreach (MonsterTargetEffectNode child in plan.Children)
                    {
        ...
        private bool QueueMonsterActiveSkillProjectile(
            ...
            string source)
        {
            if (monster == null || target?.PlayerState == null || projectile?.Impact == null)
                return false;
        ...
        private void TickMonsterActiveSkillProjectile(PendingMonsterProjectileImpact pending, int projectileIndex, int nowTick)
        {
            ...
            List<(CombatPlayer Player, int ImpactDistanceFixed)> hitTargets = FindMonsterProjectilePlayerHits(
                pending,
                segmentStartFixedX,
                segmentStartFixedY,
                nextFixedX,
                nextFixedY,
                pathDistanceFixed);
            foreach ((CombatPlayer Player, int ImpactDistanceFixed) candidate in hitTargets)
            {
                if (candidate.ImpactDistanceFixed > worldImpactDistanceFixed)
                    break;
                CombatPlayer hitTarget = candidate.Player;
                if (GetPlayer(hitTarget.EntityId) != hitTarget
                    || !hitTarget.IsAlive
                    || hitTarget.PlayerState == null
                ...
                hitTarget.IsAlive = hitTarget.PlayerState.CurrentHPWire > 0;
                if (handled && !deferred)
                    OnMonsterAttackResolved?.Invoke(monster, hitTarget, hpShifted, hitTarget.PlayerState.CurrentHPWire);
        ...
        private void UpdateMonsterActiveSkillProjectileSeeking(PendingMonsterProjectileImpact pending)
        {
            if (pending == null || !pending.SeekTargets || pending.TurnRateF32 <= 0)
                return;
            CombatPlayer seekTarget = GetPlayer(pending.TargetEntityId);
            if (seekTarget == null
                || !seekTarget.IsAlive
                || seekTarget.PlayerState == null
                || !string.Equals(RoomRuntime.NormalizeInstanceKey(seekTarget.InstanceKey), pending.InstanceKey, StringComparison.OrdinalIgnoreCase))
            {
                ...
                foreach (CombatPlayer candidate in GetPlayersInEntityOrder())
                {
                    if (candidate == null
                        || !candidate.IsAlive
        ...
        private List<(CombatPlayer Player, int ImpactDistanceFixed)> FindMonsterProjectilePlayerHits(
        ...
        {
            var candidates = new List<(CombatPlayer Player, int ImpactDistanceFixed)>();
            ...
            foreach (CombatPlayer player in GetPlayersInEntityOrder())
            {
                if (player == null
                    ...
                    continue;
                int sizeScalePercent = Math.Max(0, 100 + player.PlayerState.GetActiveAttributeModifierValue("SIZEMOD"));
                int avatarRadiusF32 = (int)(((long)ResolveAvatarUnitBehaviorRadiusF32() * sizeScalePercent) / 100L);
        ...
        private bool QueueMonsterActiveSkillChain(
            ...
            string source)
        {
            if (monster == null || target?.PlayerState == null || plan?.Impact == null)
                return false;
            ...
            if (!plan.SkipSourceChain)
            {
                ...
                if (handled && !nestedDeferred)
                    OnMonsterAttackResolved?.Invoke(monster, target, hpShifted, target.PlayerState.CurrentHPWire);
            }
        ...
        public void TickMonsterChainSubEntity(long sequence, string instanceKey, uint simulationTick)
        {
            ...
            for (int branchIndex = 0; branchIndex < branchCountAtEntry; branchIndex++)
            {
                ...
                CombatPlayer branchTarget = GetPlayer(branch.TargetEntityId);
                if (branchTarget == null
                    || branchTarget.PlayerState == null
            ...
            int simulationTick)
        {
            ...
            var candidates = new List<(CombatPlayer Player, long DistanceSquaredFixed8)>();
            ...
            {
                CombatPlayer next = candidates[candidateIndex].Player;
                pending.Branches.Add(new MonsterChainBranch
                {
                ...
                next.IsAlive = next.PlayerState.CurrentHPWire > 0;
                if (handled && !nestedDeferred)
                    OnMonsterAttackResolved?.Invoke(pending.SourceMonster, next, hpShifted, next.PlayerState.CurrentHPWire);
        ...
        private bool PassesMonsterTargetEffectChance(Monster sourceMonster, CombatPlayer target, string effectPath, int chanceWire)
        {
        ...
        private List<CombatPlayer> CollectMonsterEnemyAoeTargets(Monster sourceMonster, CombatPlayer center, int radiusF32, int maxTargets)
        {
            var ranked = new List<(CombatPlayer Player, long DistanceSquaredF32)>();
            if (sourceMonster == null || center == null || radiusF32 <= 0 || maxTargets <= 0)
                return new List<CombatPlayer>();
            long radiusSquaredF32 = ((long)radiusF32 * radiusF32) >> 8;
            foreach (CombatPlayer candidate in GetPlayersInEntityOrder())
            {
                if (candidate == null || !candidate.IsAlive || candidate.PlayerState == null || !MatchesInstance(sourceMonster, candidate.InstanceKey))
                    continue;
                long dx = (long)candidate.PosFixedX - center.PosFixedX;
                long dy = (long)candidate.PosFixedY - center.PosFixedY;
            ...
            var result = new List<CombatPlayer>(count);
        ...
        private bool ApplyMonsterTargetAttributeModifier(
            ...
            string source)
        {
            if (sourceMonster == null || target?.PlayerState == null || modifier == null)
                return false;
        ...
        private bool ApplyMonsterSpellDamageEffect(Monster monster, CombatPlayer target, MonsterSpellDamagePlan damage, string marker, string source)
        {
            if (monster == null || target?.PlayerState == null || damage == null || !target.IsAlive)
                return false;
            MersenneTwister rng = GetRoomRngForMonster(monster);
            if (rng == null)
                return false;
            ...
            int minDamage;
            int maxDamage;
            if (string.Equals(damage.AttackType, "RANGED", StringComparison.OrdinalIgnoreCase))
                DamageResolver.ComputeRangedDamageRange(Math.Max(1, (int)monster.Level), ResolveMonsterSpellAgility(monster), Math.Max(0x100, monster.WeaponDamageF32), damage.DamageModF32, damage.DamageVolatilityF32, out minDamage, out maxDamage);
            else
                DamageResolver.ComputeSpellDamageRange(Math.Max(1, (int)monster.Level), ResolveMonsterSpellIntellect(monster), damage.DamageModF32, damage.DamageVolatilityF32, out minDamage, out maxDamage);
            uint damageRaw = 0;
            ...
            if (adjustedDamageWire > 0)
            {
                ...
                if (damage.DamageKind != 3 && appliedDamage > 0)
                    ApplyMonsterOnDamageCallback(monster, appliedDamage, source ?? marker ?? "SpellDamageEffect");
            }
            target.PlayerState.DoAttributeModifierDamageEvent(rng, target.EntityId, target.Name, source ?? marker ?? "SpellDamageEffect");
            if (afterHP == 0)
            {
                TryCommitPlayerAttributeModifierDeathEvent(target, source ?? marker ?? "SpellDamageEffect");
            ...
            Debug.LogError($"[SPELL-DAMAGE] source={monster.Name}#{monster.EntityId} target={target.Name}#{target.EntityId} result={result} skill={damage.SkillPath} effect={damage.EffectPath} attackType={damage.AttackType} damageType={damage.DamageType}/{damage.DamageTypeId} range=[{minDamage},{maxDamage}] damageRaw=0x{damageRaw:X8} preQueryWire={damageWire} damageWire={adjustedDamageWire} hp={beforeHP}->{afterHP}/{target.PlayerState.MaxHPWire} criticalRaw=0x{criticalRaw:X8} criticalThreshold={criticalThreshold} effectRaw=0x{effectRaw:X8} rngPos={rng.CallsSinceReseed} sourceFunction=SpellDamageEffect::doEffect@0x0054FD20 Damage::apply@0x004F6580");
