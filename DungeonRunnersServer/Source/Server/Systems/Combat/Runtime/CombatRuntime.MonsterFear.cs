    ...
    public partial class CombatRuntime
    {
        ...
        private static bool AreMonstersFriendsForAuthoredAoe(Monster source, Monster candidate)
        {
            if (source == null || candidate == null)
                return false;
            if (source.FactionID != 0 || candidate.FactionID != 0)
                return source.FactionID == candidate.FactionID;
            return !string.IsNullOrWhiteSpace(source.Faction)
                && string.Equals(source.Faction, candidate.Faction, StringComparison.OrdinalIgnoreCase);
        }
