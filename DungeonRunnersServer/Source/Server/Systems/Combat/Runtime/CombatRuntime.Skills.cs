...
public partial class CombatRuntime
    {        private bool TryResolveMonsterStunActionSkillEffect(Monster monster, out MonsterStunActionSkillEffect effect)
        ...
        private void TraceMonsterState(Monster monster, string phase, CombatPlayer target = null, int distFixed = -1, int rangeFixed = -1, string reason = null)
        {
            ...
            out bool completeAfterResidualMove)
        {
            ...
            while (monster.UnitMoverPathIndex < monster.UnitMoverPathFixed.Count)
            {
                ...
                if (waypointFixedX != curFixedX || waypointFixedY != curFixedY)
                    break;
            ...
            if (monster.UnitMoverPathIndex == monster.UnitMoverPathFixed.Count - 1)
            {
                ...
                long terminalDistanceSquaredFixed = terminalDeltaFixedX * terminalDeltaFixedX + terminalDeltaFixedY * terminalDeltaFixedY;
                long speedPerFrameSquaredFixed = (long)speedPerFrameFixed * speedPerFrameFixed;
                long steeringArrivalSquaredFixed = (long)steeringArrivalDistanceFixed * steeringArrivalDistanceFixed;
            ...
            if (monster.UnitMoverPathIndex < monster.UnitMoverPathFixed.Count - 1)
            {
                ...
                if (!collided)
                {
                    int previousPathIndex = monster.UnitMoverPathIndex;
                    monster.UnitMoverPathIndex++;
                    targetFixedX = nextFixedX;
                    targetFixedY = nextFixedY;
                ...
                else
                {
                    ...
                    long intermediateDistanceSquaredFixed = intermediateDeltaFixedX * intermediateDeltaFixedX + intermediateDeltaFixedY * intermediateDeltaFixedY;
                    long speedPerFrameSquaredFixed = (long)speedPerFrameFixed * speedPerFrameFixed;
                    long steeringArrivalSquaredFixed = (long)steeringArrivalDistanceFixed * steeringArrivalDistanceFixed;
        ...
        private bool TryCommitPendingMonsterFollowMovement(Monster monster, bool emitPositionChanged)
        {
            if (monster == null || !monster.FollowMovementCommitPending)
                return false;
            if (monster.Behavior?.FollowMovesTowardTarget != true
                || !monster.MoveInDirectionHeadingInit
                || monster.UnitMoverPathOwner != 0)
            {
                monster.ClearFollowMovementCommit();
                return false;
            }
            if (monster.FollowMovementPendingCommitTick > _combatTick)
                return true;
            int pendingFixedX = monster.FollowMovementPendingFixedX;
            int pendingFixedY = monster.FollowMovementPendingFixedY;
            int pendingFixedZ = monster.FollowMovementPendingFixedZ;
            int pendingHeadingFixed = monster.FollowMovementPendingHeadingFixed;
            bool pendingMovingThisFrame = monster.FollowMovementPendingMovingThisFrame;
            int previousFixedX = monster.PosFixedX;
            int previousFixedY = monster.PosFixedY;
            monster.ClearFollowMovementCommit();
            monster.PosFixedX = pendingFixedX;
            monster.PosFixedY = pendingFixedY;
            monster.PosFixedZ = pendingFixedZ;
            monster.ChaseHeadingFixed = pendingHeadingFixed;
            monster.HeadingFixed = pendingHeadingFixed;
            monster.UnitMoverMovingThisFrame = pendingMovingThisFrame;
            monster.ClientVisibleMovingThisFrame = pendingMovingThisFrame
                || pendingFixedX != previousFixedX
                || pendingFixedY != previousFixedY;
            monster.State = MonsterState.Chase;
            if (emitPositionChanged)
                OnMonsterPositionChanged?.Invoke(monster);
            TraceMonsterState(monster, "movement", null, 0, 0, "unitmover-commit");
            return true;
        }

        private static void QueuePendingMonsterFollowMovement(
            Monster monster,
            uint commitTick,
            int fixedX,
            int fixedY,
        ...
        {
            monster.FollowMovementPendingCommitTick = commitTick;
            monster.FollowMovementPendingFixedX = fixedX;
            monster.FollowMovementPendingFixedY = fixedY;
            monster.FollowMovementPendingFixedZ = fixedZ;
            monster.FollowMovementPendingHeadingFixed = headingFixed;
            monster.FollowMovementPendingMovingThisFrame = movingThisFrame;
            monster.FollowMovementCommitPending = true;
            monster.FollowMovementCommitArmed = false;
            monster.ChaseHeadingFixed = headingFixed;
            monster.HeadingFixed = headingFixed;
            monster.UnitMoverMovingThisFrame = movingThisFrame;
            monster.ClientVisibleMovingThisFrame = movingThisFrame;
            monster.State = MonsterState.Chase;
        }
        ...
        private void ProcessMonsterMovement(uint playerEntityId, Monster onlyMonster, bool emitPositionChanged)
        {
            foreach (var monster in SelectMonsters(onlyMonster))
            {
                ...
                if (!monster.IsAlive || (!monster.AggroTriggered && !useTargetMove && !followAction) || movementTargetId == 0) continue;
                bool previousUnitMoverMovingThisFrame = monster.UnitMoverMovingThisFrame;
                monster.UnitMoverMovingThisFrame = false;
                if (playerEntityId != 0 && movementTargetId != playerEntityId) continue;
                if (!_players.TryGetValue(movementTargetId, out var target)
                    || target == null
                    || !target.IsAlive
                    || target.PlayerState == null
                ...
                if (monster.FollowMovementCommitPending && TryCommitPendingMonsterFollowMovement(monster, emitPositionChanged))
                    continue;
                if (!followMove && !searchForAttackMove && !followRepositionMove && !followFindLineOfSightMove && !useTargetMove && !moveInDirection) continue;
                ...
                int leashRangeFixed = ResolveMonsterLeashRangeFixed(monster);
                if (leashRangeFixed > 0)
                {
                    TryGetMonsterClientVisiblePositionFixed(monster, 0, out int visibleFixedX, out int visibleFixedY);
                    int spawnFixedX = monster.SpawnPosFixedX;
                    int spawnFixedY = monster.SpawnPosFixedY;
                    long lhx = (long)visibleFixedX - spawnFixedX;
                    long lhy = (long)visibleFixedY - spawnFixedY;
                    long leashSq = (long)leashRangeFixed * leashRangeFixed;
                    if ((lhx * lhx) + (lhy * lhy) > leashSq)
                    {
                        ...
                        HealMonsterToFullOnIdle(monster, "leash-return");
                        WanderSimulator.Instance.RegisterMonster(monster, !string.IsNullOrWhiteSpace(monster.EncounterGroupKey));
                        continue;
                    }
                }

                int allowedRangeFixed = useTargetMove
                    ? ResolveMonsterSkillTargetClearRangeF32(monster, monster.SelectedActiveSkill)
                    : ResolveMonsterEffectiveAttackRangeFixed(monster);
                bool pointMove = searchForAttackMove || followRepositionMove || followFindLineOfSightMove;
                bool directionMove = moveInDirection && !pointMove && !useTargetMove;
                if (!directionMove && allowedRangeFixed <= 0) continue;
                int movementTargetFixedX;
                int movementTargetFixedY;
                if (searchForAttackMove)
                {
                ...
                bool movedThisFrame = curFixedX != movementStartFixedX || curFixedY != movementStartFixedY;
                monster.ClientVisibleMovingThisFrame = curFixedX != movementStartFixedX || curFixedY != movementStartFixedY;
                int resolvedFixedZ = chasePm != null && chasePm.TryGetHeightAtFixed(curFixedX, curFixedY, out _)
                    ? curFixedZ
                    : ResolveTerrainHeightFixedValue(monster, curFixedX, curFixedY, curFixedZ);
                if (followMove && directionMove && monster.FollowMovementCommitArmed && movedThisFrame)
                {
                    QueuePendingMonsterFollowMovement(
                        monster,
                        unchecked(_combatTick + 1),
                        curFixedX,
                        curFixedY,
                        resolvedFixedZ,
                        monster.ChaseHeadingFixed,
                        monster.UnitMoverMovingThisFrame);
                    TraceMonsterState(monster, "movement", target, distFixed, allowedRangeFixed, "unitmover-pending-commit");
                    continue;
                }
        ...
        private Behavior.MonsterBehavior2.ActionSlot TickMonsterUpdateSkillsTimer(Monster monster)
        {
            ...
            for (int skillIndex = 0; skillIndex < monster.ActiveSkills.Count; skillIndex++)
            {
                ...
                List<(CombatPlayer Player, long DistanceSquaredFixed8)> candidates = CollectMonsterActiveSkillEnemyCandidates(monster, skill);
                ...
                CombatPlayer selectedTarget = candidates[selectedIndex].Player;
                ApplyMonsterActiveSkillSelection(monster, skill);
