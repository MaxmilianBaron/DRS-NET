...
public partial class CombatRuntime
    {        private static int ResolveMonsterActiveSkillMinimumRangeF32(MonsterActiveSkillRuntime skill)
        ...
        private List<(CombatPlayer Player, long DistanceSquaredFixed8)> CollectMonsterActiveSkillEnemyCandidates(Monster monster, MonsterActiveSkillRuntime skill)
        {
            ...
            foreach (CombatPlayer player in GetPlayersInEntityOrder())
            {
                ...
                if (!MatchesInstance(monster, player.InstanceKey))
                    continue;
                if (!PassesMonsterActiveSkillHealthPct(player.PlayerState.CurrentHPWire, player.PlayerState.MaxHPWire, true, skill.TargetHealthPctF32))
                    continue;
        ...
        private void ProcessMonsterActiveSkillEffect(Monster monster)
        {
            ...
            if (elapsed == triggerTicks)
            {
                ...
                if (_players.TryGetValue(targetEntityId, out CombatPlayer target)
                    && target != null
                    && target.IsAlive
        ...
        private void ProcessMonsterAttacks(Monster onlyMonster)
        {
            foreach (var monster in SelectMonsters(onlyMonster))
            {
                ...
                if (!_players.TryGetValue(runtimeTargetId, out var target) || !target.IsAlive || target.PlayerState == null) continue;
        ...
        public bool CommitMonsterAttackActionInput(Monster monster, CombatPlayer target, byte sessionId, uint applyTick, bool replaceCurrentAction = false, bool useTargetAction = false)
        {
        ...
        public bool CommitMonsterAttackActionWireInput(Monster monster, CombatPlayer target, byte sessionId, uint applyTick)
        {
            if (monster == null || target == null || !monster.IsAlive || !target.IsAlive)
                return false;
            if (!monster.AttackPending || monster.AttackSessionId != sessionId)
        ...
        private void FaceMonsterAttackTarget(Monster monster, CombatPlayer target)
        {
            if (monster == null || target == null)
                return;
            TryGetMonsterClientVisiblePositionFixed(monster, target.EntityId, out int monsterFixedX, out int monsterFixedY);
        ...
        private void ArmMonsterRuntimeAttack(Monster monster, CombatPlayer target, string marker, uint applyTick)
        {
