...
public partial class CombatRuntime
    {        private void UpdateMonsterWorldEntityAnimationSnapshot(Monster monster)
        ...
        private void TraceFarTargetAction(Monster monster, CombatPlayer target, int distFixed, int allowedRangeFixed, string source)
        {
        ...
        private int ResolveMonsterDamageTableF32(byte level)
        {
            return GCDatabase.Instance.RequireCurveValueFixed32("MonsterDamage", Math.Clamp((int)level, 1, 110));
        }
        ...
        public bool ApplySpellKnockDownEffectToMonster(MersenneTwister rng, Monster target, int attackerLevel, int strength, int chanceWire, string marker, uint sourceEntityId = 0, bool useWeaponImpactPosition = false)
        {
        ...
        private static DamageQueryResult ApplyPlayerDamageQueryWire(uint damageWire, CombatPlayer target, Monster attacker, int damageTypeId, byte damageKind, string source, int attackerLevel, MersenneTwister rng, bool resistable)
        {
        ...
        private WeaponDamageInput CreateMonsterWeaponDamageInput(Monster monster, CombatPlayer target, MersenneTwister rng, string source)
        {
            ...
            int baseDamageF32 = monster != null ? ResolveMonsterDamageTableF32(monster.Level) : 0x100;
            ...
            return new WeaponDamageInput
            {
                ...
                DamageBonus = 0,
                DamageMod = Math.Max(0, damageModPercent),
        ...
        private bool TryDeferMonsterProjectileImpact(Monster monster, CombatPlayer target, int distF32, string marker, string source)
        {
        ...
        private void ResolveMonsterAttackDamage(Monster monster, CombatPlayer target, int distF32, string marker, string source, bool applyActiveSkillEffect = true)
        {
            TryGetMonsterClientVisiblePositionFixed(monster, target.EntityId, out int clientVisibleMonsterX, out int clientVisibleMonsterY);
            int monsterAttackRangeF32 = ResolveMonsterEffectiveAttackRangeFixed(monster);
            ...
            if (monsterAttackRangeF32 > 0)
            {
                ResolveMonsterActionTargetPositionFixed(target, out int targetFixedX, out int targetFixedY, out _);
                clientVisibleInContact = EvaluateUseTargetInitUseFixed(clientVisibleMonsterX, clientVisibleMonsterY, targetFixedX, targetFixedY, monsterAttackRangeF32, CLIENT_CONTACT_RANGE_EPSILON_FIXED, out _, out clientVisibleDistSqFixed8, out clientVisibleRangeSqFixed8);
            }
            if (DungeonRunners.Core.RuntimeEvidence.TrackingEnabled)
                DungeonRunners.Core.RuntimeEvidence.LogForPlayer(target.Name, "[MON-HIT]", $"monster={monster.Name}#{monster.EntityId} distF32={distF32} cvDistSq={clientVisibleDistSqFixed8} rangeSq={clientVisibleRangeSqFixed8} cvMonFixed8=({clientVisibleMonsterX},{clientVisibleMonsterY}) avatarFixed8=({target.PosFixedX},{target.PosFixedY}) attackRangeF32={monsterAttackRangeF32} inContact={clientVisibleInContact} projectile={projectileImpact} targetHp={target.PlayerState.CurrentHPWire} source={source ?? marker}");
            if (!projectileImpact && !clientVisibleInContact)
            {
                if (DungeonRunners.Core.RuntimeEvidence.TrackingEnabled)
                    DungeonRunners.Core.RuntimeEvidence.LogForPlayer(target.Name, "[MON-HIT-SKIP]", $"monster={monster.Name}#{monster.EntityId} cvDistSq={clientVisibleDistSqFixed8} rangeSq={clientVisibleRangeSqFixed8} attackRangeF32={monsterAttackRangeF32} reason=client-visible-out-of-contact source={source ?? marker}");
            ...
            if (hit && !blocked)
            {
                ...
                if (adjustedDamageWire == 0)
                {
                    ...
                    target.PlayerState.DoAttributeModifierDamageEvent(rng, target.EntityId, target.Name, source ?? marker ?? "Weapon::applyDamage");
                    if (sameHPWire == 0)
                        TryCommitPlayerAttributeModifierDeathEvent(target, source ?? marker ?? "Weapon::applyDamage");
                    OnMonsterAttackResolved?.Invoke(monster, target, false, sameHPWire);
                }
                else
                {
                    target.PlayerState.TakeQueriedDamage(adjustedDamageWire);
                    ...
                    Debug.LogError($"[{marker}] {monster.Name}#{monster.EntityId}->{target.Name} HIT source={source} damageWire={adjustedDamageWire} preQueryWire={damageWire} hpWire={currentHPWire}->{newHPWire}/{target.PlayerState.MaxHPWire} rangeWire=[{minDamage},{maxDamage}] avgWire={averageDamage} ar={attackRating} dr={defenseRating} levels={attackerLevel}->{defenderLevel} hitThreshold={hitThreshold} rngPos={rng.CallsSinceReseed} anim={monster.AttackAnimationIndex} use=0x{monster.AttackUseRaw:X8} sound=0x{monster.AttackSoundRaw:X8} soundGate=0x{monster.AttackSoundGateRaw:X8} soundRepeat=0x{monster.AttackSoundRepeatRaw:X8} hit=0x{hitRaw:X8}/{hitRoll} block=0x{blockRaw:X8}/{blockRoll}/{blockChance} dmgRaw=0x{damageRaw:X8} distF32={distF32} result={query.ResultName}");
                    ...
                    OnMonsterAttackResolved?.Invoke(monster, target, true, target.PlayerState.CurrentHPWire);
                }
            }
            else if (blocked)
            {
        ...
        private bool TryApplyMonsterActiveSkillEffect(Monster monster, CombatPlayer target, int distF32, string marker, string source, out bool handled, out bool hpShifted)
        {
            handled = false;
            hpShifted = false;
            ...
            if (!stackAccepted)
            {
                ...
                Debug.LogError($"[SPELL-MOD] skill={effect.SkillPath} effect={effect.EffectPath} modifier={effect.ModifierPath} attr=HIT_POINT_REGEN_BONUS value={effect.HitPointRegenBonus} durationF32={effect.DurationF32} durationTicks={effect.DurationTicks} applied=False hp={beforeHP}->{rejectedHP}/{target.PlayerState.MaxHPWire} source={source ?? marker ?? "unknown"} sourceFunction=SpellModEffect::doEffect@0x00554460 chanceDraw=false stack={effect.StackRule ?? ""} stackAccepted=False");
                return true;
            }
            ...
            Debug.LogError($"[SPELL-MOD] skill={effect.SkillPath} effect={effect.EffectPath} modifier={effect.ModifierPath} attr=HIT_POINT_REGEN_BONUS value={effect.HitPointRegenBonus} durationF32={effect.DurationF32} durationTicks={effect.DurationTicks} power={powerLevel} applied={applied} hp={beforeHP}->{afterHP}/{target.PlayerState.MaxHPWire} source={source ?? marker ?? "unknown"} sourceFunction=SpellModEffect::doEffect@0x00554460 chanceDraw=false stack={effect.StackRule ?? ""} stackAccepted=True");
            return true;
        }
        ...
        private bool ApplyMonsterWeaponDamageSkillEffect(Monster monster, CombatPlayer target, MonsterWeaponDamageSkillEffect effect, int distF32, string marker, string source)
        {
            return ApplyMonsterWeaponDamageSkillEffect(monster, target, effect, distF32, marker, source, out _);
        }

        private bool ApplyMonsterWeaponDamageSkillEffect(Monster monster, CombatPlayer target, MonsterWeaponDamageSkillEffect effect, int distF32, string marker, string source, out bool landed)
        {
            landed = false;
            RoomRuntime runtime = GetRoomRuntimeForMonster(monster);
            MersenneTwister rng = runtime?.RoomRng;
            ...
            if (damageResult.IsHit && !damageResult.IsBlocked)
            {
                ...
                if (adjustedDamageWire == 0)
                {
                    ...
                    target.PlayerState.DoAttributeModifierDamageEvent(rng, target.EntityId, target.Name, source ?? marker ?? "SpellWeaponDamageEffect");
                    if (sameHP == 0)
                        TryCommitPlayerAttributeModifierDeathEvent(target, source ?? marker ?? "SpellWeaponDamageEffect");
                }
                else
                {
                    target.PlayerState.TakeQueriedDamage(adjustedDamageWire);
                    ...
                    bool modifierApplied = effect.DamageModifier != null && target.PlayerState.CurrentHPWire > 0 && ApplyPlayerDamageModifierFromMonster(monster, target, effect.DamageModifier, source ?? marker ?? "SpellWeaponDamageEffect");
                    Debug.LogError($"[SPELL-WEAPON-DAMAGE] {monster.Name}#{monster.EntityId}->{target.Name} HIT skill={effect.SkillPath} effect={effect.WeaponEffectPath} damageWire={adjustedDamageWire} preQueryWire={damageWire} hpWire={beforeHP}->{afterHP}/{target.PlayerState.MaxHPWire} arMod={effect.AttackRatingMod} ar={baseAttackRating}->{damageInput.AttackRating} dmgMod={baseDamageMod}->{damageInput.DamageMod} hit=0x{damageResult.HitRaw:X8}/{damageResult.HitRoll} block=0x{damageResult.BlockRaw:X8}/{damageResult.BlockRoll}/{damageResult.BlockChance} dmgRaw=0x{damageResult.DamageRaw:X8} effectRaw=0x{effectRaw:X8} modifierApplied={modifierApplied} distF32={distF32} rngPos={rng.CallsSinceReseed} result={query.ResultName} sourceFunction=Weapon::applyDamage@0x00597E50");
            ...
            {
                Debug.LogError($"[SPELL-WEAPON-DAMAGE] {monster.Name}#{monster.EntityId}->{target.Name} BLOCK skill={effect.SkillPath} effect={effect.WeaponEffectPath} hpWire={beforeHP}->{beforeHP}/{target.PlayerState.MaxHPWire} arMod={effect.AttackRatingMod} ar={baseAttackRating}->{damageInput.AttackRating} dmgMod={baseDamageMod}->{damageInput.DamageMod} hit=0x{damageResult.HitRaw:X8}/{damageResult.HitRoll} block=0x{damageResult.BlockRaw:X8}/{damageResult.BlockRoll}/{damageResult.BlockChance} distF32={distF32} rngPos={rng.CallsSinceReseed} sourceFunction=Weapon::applyDamage@0x00597E50");
            ...
            {
                Debug.LogError($"[SPELL-WEAPON-DAMAGE] {monster.Name}#{monster.EntityId}->{target.Name} MISS skill={effect.SkillPath} effect={effect.WeaponEffectPath} hpWire={beforeHP}->{beforeHP}/{target.PlayerState.MaxHPWire} arMod={effect.AttackRatingMod} ar={baseAttackRating}->{damageInput.AttackRating} dmgMod={baseDamageMod}->{damageInput.DamageMod} hit=0x{damageResult.HitRaw:X8}/{damageResult.HitRoll} threshold={damageResult.HitThreshold} block=0x{damageResult.BlockRaw:X8}/{damageResult.BlockRoll}/{damageResult.BlockChance} distF32={distF32} rngPos={rng.CallsSinceReseed} sourceFunction=Weapon::applyDamage@0x00597E50");
