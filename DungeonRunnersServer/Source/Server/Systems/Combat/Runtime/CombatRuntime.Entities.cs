...
public partial class CombatRuntime
    {public int GetComponentOffset(uint componentId)
        ...
        private static bool IsProjectileEnemyMonster(Monster monster)
        {
            if (monster == null)
                return false;
            if (monster.FactionID < 0)
                return false;

            string faction = monster.Faction;
            if (string.IsNullOrWhiteSpace(faction))
                return true;
            faction = faction.Trim();
            if (faction.Equals("player", StringComparison.OrdinalIgnoreCase)
                || faction.Equals("players", StringComparison.OrdinalIgnoreCase)
                || faction.Equals("hero", StringComparison.OrdinalIgnoreCase)
                || faction.Equals("friendly", StringComparison.OrdinalIgnoreCase)
                || faction.Equals("ally", StringComparison.OrdinalIgnoreCase)
                || faction.Equals("neutral", StringComparison.OrdinalIgnoreCase)
                || faction.Equals("npc", StringComparison.OrdinalIgnoreCase)
                || faction.Equals("town", StringComparison.OrdinalIgnoreCase)
                || faction.IndexOf("friendly", StringComparison.OrdinalIgnoreCase) >= 0
                || faction.IndexOf("neutral", StringComparison.OrdinalIgnoreCase) >= 0
                || faction.IndexOf("vendor", StringComparison.OrdinalIgnoreCase) >= 0)
                return false;
            return true;
        }
        ...
        private int ResolveMonsterSkillTargetClearRangeF32(Monster monster, MonsterActiveSkillRuntime skill)
        {
            ...
            int targetRadiusF32 = ResolveAvatarBoundingBoxRadiusXYF32();
            int scaledActorRadiusF32 = (int)(((long)actorRadiusF32 * 0x180) >> 8);
            int scaledTargetRadiusF32 = (int)(((long)targetRadiusF32 * 0x180) >> 8);
            return SaturatingAddFixed8(
                skillRangeF32,
        ...
        private int ResolveClientContactRangeFixed(Monster monster, CombatPlayer player, int allowedRangeFixed)
        {
        ...
        private bool TryGetMonsterClientVisibleDistanceFixed(Monster monster, CombatPlayer target, out int distanceFixed, out long distanceSqFixed8)
        {
            distanceFixed = 0;
            distanceSqFixed8 = 0;
        ...
        private void RefreshMonsterCombatContact(Monster monster, CombatPlayer target)
        {
            if (monster == null || target == null) return;
        ...
        private bool IsClientCombatContactFixed(Monster monster, CombatPlayer target, int allowedRangeFixed)
        {
        ...
        private bool IsMonsterActionTargetClearFixed(Monster monster, CombatPlayer target, int allowedRangeFixed)
        {
            if (target == null)
                return false;
            ResolveMonsterActionTargetPositionFixed(target, out int targetFixedX, out int targetFixedY, out _);
        ...
        private bool EvaluateMonsterActionTargetClearFixed(Monster monster, CombatPlayer target, int allowedRangeFixed, int targetFixedX, int targetFixedY)
        {
        ...
        private bool HasMonsterTargetAction(Monster monster, CombatPlayer target, int distFixed)
        {
            if (monster == null || target == null) return false;
        ...
        private bool HasMonsterWeaponRuntimeReachFixed(Monster monster, CombatPlayer target, int allowedRangeFixed, bool clientContact)
        {
            if (monster == null || target == null) return false;
