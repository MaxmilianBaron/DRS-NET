    ...
    public partial class CombatRuntime
    {
            ...
            string source)
        {
            if (monster == null
                ...
                || !monster.IsAlive
                || !target.IsAlive
                || target.PlayerState.CurrentHPWire == 0)
                return false;
        ...
        private bool ExecuteMonsterDeathProcEffectNode(MonsterTargetEffectNode plan, Monster sourceMonster, string source)
        {
            ...
            if (plan.Kind == MonsterTargetEffectNodeKind.EnemyAoe)
            {
                if (!PassesMonsterTargetEffectChance(sourceMonster, null, plan.EffectPath, plan.ChanceWire))
                    return true;
                List<CombatPlayer> targets = CollectMonsterAuraEnemyTargets(sourceMonster, plan.RadiusF32, plan.MaxTargets);
                bool handled = true;
                foreach (CombatPlayer target in targets)
                {
                    ...
                    if (hpShifted)
                        OnPlayerDamageResolved?.Invoke(sourceMonster, target, true, target.PlayerState.CurrentHPWire, source ?? "ProcModifier::DoDeathProc");
                }
                return handled;
            }
