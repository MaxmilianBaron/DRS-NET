    ...
    public partial class CombatRuntime
    {
            ...
            string source)
        {
            if (sourceMonster == null
                || anchor?.PlayerState == null
                || !anchor.IsAlive
                || anchor.PlayerState.CurrentHPWire == 0
            ...
            long order = replace ? existing.Order : ++_nextPlayerAnchoredMonsterAuraOrder;
            if (!replace)
        ...
        public void AdvancePlayerAnchoredMonsterAurasForEntity(uint anchorEntityId)
        {
            ...
            foreach (KeyValuePair<string, ActivePlayerAnchoredMonsterAura> entry in entries)
            {
                if (!_activePlayerAnchoredMonsterAuras.TryGetValue(entry.Key, out ActivePlayerAnchoredMonsterAura aura)
                    || aura?.Plan == null)
                    continue;
                CombatPlayer anchor = GetPlayer(aura.AnchorEntityId);
                ...
                if ((!anchor.IsAlive || anchor.PlayerState == null || anchor.PlayerState.CurrentHPWire == 0)
                ...
                if (aura.ScanCountdownTicks != 0)
                    continue;
                List<CombatPlayer> targets = CollectPlayerAnchoredMonsterAuraFriendTargets(anchor, aura.Plan.RangeF32);
                ...
                foreach (CombatPlayer target in targets)
        ...
        private List<CombatPlayer> CollectPlayerAnchoredMonsterAuraFriendTargets(CombatPlayer anchor, int rangeF32)
        {
            var result = new List<CombatPlayer>();
            if (anchor?.PlayerState == null || rangeF32 <= 0)
                return result;
            if (anchor.IsAlive && anchor.PlayerState.CurrentHPWire > 0)
                result.Add(anchor);
            ...
            foreach (CombatPlayer candidate in GetPlayersInEntityOrder())
            {
                if (candidate == null
                    ...
                    || candidate.PlayerState.CurrentHPWire == 0
            ...
            foreach ((CombatPlayer Player, long DistanceSquaredF32) candidate in ranked)
        ...
        private void AdvanceMonsterAuraModifierTick(Monster sourceMonster)
        {
            ...
            foreach (KeyValuePair<string, ActiveMonsterAura> entry in entries)
            {
                ...
                if (aura.ScanCountdownTicks != 0)
                    continue;
                List<CombatPlayer> targets = CollectMonsterAuraEnemyTargets(sourceMonster, aura.Plan.RangeF32, aura.Plan.MaxTargets);
        ...
        private List<CombatPlayer> CollectMonsterAuraEnemyTargets(Monster sourceMonster, int rangeF32, int maxTargets)
        {
            var ranked = new List<(CombatPlayer Player, long DistanceSquaredF32)>();
            if (sourceMonster == null || rangeF32 <= 0 || maxTargets <= 0)
        ...
        private void RemoveMonsterAurasFromSource(uint sourceEntityId, string source)
        {
            ...
            foreach (KeyValuePair<string, ActivePlayerAnchoredMonsterAura> removal in anchoredRemovals)
                RemovePlayerAnchoredMonsterAura(
                    removal.Key,
                    removal.Value,
                    GetPlayer(removal.Value.AnchorEntityId),
        ...
        private void RemovePlayerAnchoredMonsterAurasForAnchor(uint anchorEntityId, string source)
        {
            ...
            removals.Sort((left, right) => left.Value.Order.CompareTo(right.Value.Order));
            CombatPlayer anchor = GetPlayer(anchorEntityId);
