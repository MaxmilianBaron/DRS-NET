...
namespace DungeonRunners.Combat
{
    ...
    {private static CombatRuntime _instance;
        ...
        {
            return monster != null && simulationTick >= monster.EntityUpdateAdmissionTick;
        }
        ...
        public event Action<Monster, CombatPlayer> OnMonsterFollowReachedNearStop;
        ...
        private static void ResolveMonsterBehaviorTargetPositionFixed(Monster monster, CombatPlayer target, out int posFixedX, out int posFixedY, out int posFixedZ)
        {
            if (target == null)
            {
                posFixedX = 0;
        ...
        private static void ResolveSearchForAttackTargetPositionFixed(CombatPlayer target, out int posFixedX, out int posFixedY, out int posFixedZ)
        {
            if (target == null)
            {
                posFixedX = 0;
        ...
        private static void ResolveMonsterFollowTargetStateFixed(CombatPlayer target, out int posFixedX, out int posFixedY, out int posFixedZ, out bool movingThisFrame)
        {
            if (target == null)
            {
                posFixedX = 0;
        ...
        private static void ResolveMonsterActionTargetPositionFixed(CombatPlayer target, out int posFixedX, out int posFixedY, out int posFixedZ)
        {
            if (target == null)
            {
                posFixedX = 0;
        ...
        private static uint ResolveMonsterActionTargetEntityId(Monster monster)
        {
            ...
            return followTargetEntityId != 0 ? followTargetEntityId : monster.TargetId;
        }
            ...
            out bool active)
        {
            ...
            if (monster.AggroTriggered)
            {
                ...
                if (_players.TryGetValue(movementTargetId, out CombatPlayer target)
                    && target != null
                    && target.IsAlive
