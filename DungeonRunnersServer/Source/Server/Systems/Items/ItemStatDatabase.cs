...
namespace DungeonRunners.Data
{
    public class ItemStatDatabase
    {
        ...
        public List<(int Slot, string ModRef)> GetEffectiveItemWireMods(GCObject item)
        {
            ...
            if (item.HasGeneratedItemState)
                return item.GeneratedItemModifiers
                    ...
                    .ToList();
            var staticMods = GetItemWireMods(item.GCClass);
            if (staticMods.Count > 0) return staticMods;
        ...
        public GCNode RequireItemModifierDescription(string modifierRef)
        {
            if (!IsLoaded)
            ...
            if (description == null)
                throw new InvalidDataException($"Item modifier '{modifierRef}' has no Description");
