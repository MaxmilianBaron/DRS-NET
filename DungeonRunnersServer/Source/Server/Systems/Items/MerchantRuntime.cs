...

{
                            ...
                            else if (isConsumable)
                            {
                                ...
                                if (allInv != null)
                                {
                                    ...
                                    foreach (var inventoryEntry in allInv)
                                    {
                                        ...
                                        if (storedGc == searchGc && storedCount <= maxStack - buyQty)
                                        {
                                ...
                                if (foundPartial)
                                {
                                    ...
                                    itemWriter.WriteByte(0x1E);
                                    itemWriter.WriteByte(0x0B);
                                    int partialLevel = partialItem.StoredLevel >= 0 ? partialItem.StoredLevel : Math.Max(1, partialItem.GetItemRequiredLevel());
                                    partialItem.WriteInitForInventory(itemWriter, partialX, partialY, partialSlot, partialLevel, (byte)Math.Min(newCount, byte.MaxValue));
                                    server.WritePlayerEntitySynch(conn, itemWriter);
                                    itemWriter.WriteByte(0x06);
                                    purchasedItemPacket = itemWriter.ToArray();
    ...
    public static class RPGSettings
    {
        ...
        public static int ResolveBestNativeItemQuality(GCObject item, GCNode description)
        {
            ...
            int bestQuality = 1;
            foreach ((int _, string modifierRef) in itemStats.GetEffectiveItemWireMods(item))
            {
                int modifierQuality = ResolveNativeItemQuality(itemStats.RequireItemModifierDescription(modifierRef));
