...
namespace DungeonRunners.Networking
{
    public class UnitContainer
    {
        ...
        private void ProcessGetItemLocal(RRConnection conn, LEReader reader, ushort componentId)
        {
            ...
            string gcLower = item.GCClass.ToLower();
            bool isStackedCursorItem = gcLower.Contains("questitem")
                || gcLower.Contains("consumable")
                || gcLower.Contains("potion")
                || gcLower.Contains("skillbook")
                || gcLower.Contains("voucher");
            int cursorQuantity = isStackedCursorItem ? storedStackCount : 1;
            var writer = new LEWriter();
            writer.WriteByte(0x07);
            writer.WriteByte(0x35);
            writer.WriteUInt16(componentId);
