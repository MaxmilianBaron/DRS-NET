...
namespace DungeonRunners.Utilities
{
    public static class PathMapBuilder
    {
            ...
            int minFixedY)
        {
            ...
            int localMinFixedX = collision.BlockOriginX << 8;
            int localMinFixedY = collision.BlockOriginY << 8;
            int localMaxFixedX = (collision.BlockOriginX + collision.BlockGridX * collision.BlockCellSize) << 8;
            int localMaxFixedY = (collision.BlockOriginY + collision.BlockGridY * collision.BlockCellSize) << 8;
            ...
            for (int gx = gx0; gx <= gx1; gx++)
            {
                for (int gy = gy0; gy <= gy1; gy++)
                {
                    ...
                    if (!collision.TestBoundingBoxFixed(queryMinX, queryMinY, queryMinZ, queryMaxX, queryMaxY, queryMaxZ))
                        continue;
