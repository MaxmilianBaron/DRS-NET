...
namespace DungeonRunners.Core
{
    ...
    public sealed class WorldCollision
    {
        ...
        {
            var ordered = new List<CollisionBox>(cache?.HeightSurfaces.Count ?? 0);
            if (cache == null || cache.HeightSurfaces.Count == 0)
                return ordered;
            ...
            for (int cellZ = minCellZ; cellZ <= maxCellZ; cellZ++)
            {
                ...
                for (int surfaceIndex = 0; surfaceIndex < cache.HeightSurfaces.Count; surfaceIndex++)
                {
                    ...
                    if (surface == null || markedSourceOrders.Contains(surface.SourceOrder))
                        continue;
                    if (!NativeCellKeysContain(surface.NativeCellKeys, key))
                        continue;
        ...
        private static bool TryGetNativeHeightSurfaceZRange(CollisionCache cache, int pointCellX, int pointCellY, out int minCellZ, out int maxCellZ)
        {
            ...
            for (int surfaceIndex = 0; surfaceIndex < cache.HeightSurfaces.Count; surfaceIndex++)
            {
                ...
                if (surface == null)
                    continue;
                for (int cellIndex = 0; cellIndex < surface.NativeCellKeys.Count; cellIndex++)
                {
                    NativeCollisionCellKey key = surface.NativeCellKeys[cellIndex];
                    if (key.X != pointCellX || key.Y != pointCellY)
                        continue;
                    if (key.Z < minCellZ)
                        minCellZ = key.Z;
                    if (key.Z > maxCellZ)
