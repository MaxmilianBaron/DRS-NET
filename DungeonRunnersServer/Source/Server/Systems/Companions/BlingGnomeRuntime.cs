    ...
    public class BlingGnomeRuntime : MonoBehaviour
    {
        ...
        private void ResolveConversionBonus(RRConnection conn, GnomeState g)
        {
            if (_server == null || conn == null || g == null || g.ItemsConverted <= 0 || g.BonusEligibleItemsConverted <= 0)
                return;
            int chanceF32 = GCDatabase.Instance.RequireCurveValueFixed32(ITEM_GENERATOR_CHANCE_TABLE, g.ItemsConverted);
            chanceF32 = Math.Clamp(chanceF32, 0, 100 * Fixed8Scale);
            int rawIndex = RandomStreams.GlobalStaticCalls;
            uint raw = RandomStreams.GenerateGlobalStatic(
                "BlingGnomeIGTable.chance",
                $"BlingGnome:{g.EntityId}:items={g.ItemsConverted}");
            int rollF32 = unchecked((int)(raw % (uint)(100 * Fixed8Scale)));
            bool passed = rollF32 < chanceF32;
            Debug.LogError($"[GNOME-BONUS] entity={g.EntityId} items={g.ItemsConverted} stream=globalStatic rawIndex={rawIndex} raw=0x{raw:X8} rollF32={rollF32} chanceF32={chanceF32} passed={passed} generator={g.ItemGeneratorTable} source=authored-server-reconstruction");
            if (!passed)
                return;
        ...
        {
            writer.WriteUInt32(g.BlingModifierActive ? g.BlingModifierId + 1u : 0u);
        ...
        {
            if (item == null || !IsAllowedForOwner(conn, item))
                return false;
            if (item.Item == null)
            {
            ...
            bool accepted = descriptionQuality != 0 && descriptionQuality != 6;
            Debug.LogError($"[GNOME-CONVERT-FILTER] item={item.Item.GCClass} owner={item.OwnerCharacterId} requester={(_server != null ? _server.GetCharSqlIdPublic(conn) : conn?.CharSqlId ?? 0)} descriptionQuality={descriptionQuality} accepted={accepted} sourceFunction=ActivePassiveItem::getActivePassiveItemDesc->ConvertItemsToGold::States@0x00524530");
            return accepted;
        }
        ...
        private static BlingConversionContract ResolveConversionContract()
        {
            ...
            long durationTicks = ((long)durationFixed * 30) >> 8;
