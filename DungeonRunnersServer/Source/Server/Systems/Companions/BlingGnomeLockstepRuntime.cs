...
namespace DungeonRunners.Networking
{
    public sealed class BlingGnomeLockstepRuntime
    {
        ...
        {
            if (server == null || owner == null || !_states.TryGetValue(entityId, out State state) || state.Owner != owner || !state.Initialized)
                return false;
        ...
        public void TickEntity(GameServer server, uint entityId, uint simulationTick)
        {
            ...
            else if (!state.ConvertRequested && (!state.ConvertActive || state.ConvertState == 4) && state.FollowStartedTick != simulationTick)
        ...
        public void TickItemObject(GameServer server, uint entityId, uint simulationTick)
        {
            ...
            if (_retrieveItemOwners.TryGetValue(itemEntityId, out uint retrieveOwnerId)
                && _states.TryGetValue(retrieveOwnerId, out State retrieveOwner)
                && retrieveOwner.RetrieveActive
                && retrieveOwner.RetrieveState == 4
                && retrieveOwner.RetrieveItemEntityId == itemEntityId)
            {
                if (retrieveOwner.RetrieveTimer > 0)
                    retrieveOwner.RetrieveTimer--;
                server.SetDroppedItemLifecycle(itemEntityId, 2, unchecked((ushort)retrieveOwner.RetrieveTimer), 0, simulationTick);
                if (retrieveOwner.RetrieveTimer == 0)
                {
                    uint goldAmount = 0;
                    if (server.TryGetBlingDroppedItem(itemEntityId, out GameServer.DroppedItemInfo item))
                        goldAmount = item.GoldAmount;
                    bool pickedUp = server.BlingGnomeAcquireItemAsGold(retrieveOwner.Owner, itemEntityId, goldAmount);
                    Debug.LogError($"[GNOME-RETRIEVE] entity={retrieveOwner.EntityId} item={itemEntityId} tick={simulationTick} state=4->complete acquired={pickedUp} gold={goldAmount} sourceFunction=ItemObject::updateState@0x0058A410->ItemObject::AcquireItem@0x00589E70");
                    FinishRetrieve(retrieveOwner, simulationTick, pickedUp, pickedUp ? "acquired" : "pickup-failed");
                }
            }
            ...
            bool converted = server.BlingGnomeAcquireItemAsGold(conversionOwner.Owner, conversion.EntityId, conversion.GoldAmount);
            if (converted)
            {
        ...
        {
            if (!server.TryGetBlingDroppedItem(state.RetrieveItemEntityId, out GameServer.DroppedItemInfo item)
                || !CanAcquireRetrieveItem(state, item)
                || !item.IsGoldDrop
                || item.GoldAmount == 0)
            {
            ...
            if (state.RetrieveState == 10)
            {
                ...
                state.RetrieveTimer = RetrieveFlipTicks;
                server.SetDroppedItemLifecycle(state.RetrieveItemEntityId, 2, RetrieveFlipTicks, 0, simulationTick);
                Debug.LogError($"[GNOME-RETRIEVE] entity={state.EntityId} item={state.RetrieveItemEntityId} tick={simulationTick} state=10->4 itemState=1 timer={RetrieveFlipTicks} sourceFunction=RetrieveItem::States@0x0052C8E0->ItemObject::FlipAndAcquire@0x0058B360->ItemObject::initState@0x0058A1E0");
                return;
            }
        }
        ...
        private static void TickConversion(GameServer server, State state, MersenneTwister rng, uint simulationTick)
        {
            if (state.ConvertRequested)
            {
                ...
                state.AnimationState = 8;
                state.AnimationFramesRemaining = animationFrames + 2;
                var candidates = server.GetDroppedItemsNearFixed(
                    state.Owner.CurrentZoneName ?? "",
            ...
            if (!state.ConvertItemsComplete && simulationTick >= state.ConvertStateAdvanceDueTick)
                SignalConvertCompletion(server, state, rng, simulationTick, false);

            if (!state.ConvertAnimationComplete)
            {
                state.ConvertActionTimer = simulationTick < state.ConvertAnimationDueTick
                    ? (int)(state.ConvertAnimationDueTick - simulationTick)
                    : 0;
                if (simulationTick >= state.ConvertAnimationDueTick)
                {
                    state.AnimationId = 100;
                    state.AnimationState = 1;
                    state.AnimationFramesRemaining = 0;
        ...
        private static bool TryFindSearchableGold(GameServer server, State state, out ushort entityId, out GameServer.DroppedItemInfo item)
        {
            ...
            if (pathMap == null)
                return false;
            var candidates = server.GetDroppedItemsNearFixed(state.Owner.CurrentZoneName ?? "", state.Owner.InstanceId, state.FixedX, state.FixedY, state.FixedZ, SearchRadiusFixed);
            foreach (var candidate in candidates)
            {
                GameServer.DroppedItemInfo candidateItem = candidate.info;
                if (candidateItem == null
                    || candidateItem.ItemObjectState != 6
